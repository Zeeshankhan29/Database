...
with open('final-items-reliance.yaml', 'r') as params_file:
    yaml_params = yaml.safe_load(params_file) or {}
...
def meta():
    try:
        ...
        thread_m1 = threading.Thread(target=process_model, args=(model, config['Model_M1'], image_path))
        thread_m2 = threading.Thread(target=process_model, args=(model1, config['Model_M2'], image_path))
...
def predict_image():
		...
		zoom_percentage = 100 - zoom_config
                ...
                zoom_height = int(height * (zoom_percentage / 100))
                zoom_x1 = max(center_x - zoom_width // 2, 0)
                zoom_x2 = min(center_x + zoom_width // 2, width)
                zoom_y1 = max(center_y - zoom_height // 2, 0)
                zoom_y2 = min(center_y + zoom_height // 2, height)

                # Crop zoomed region
                zoomed_region = image[zoom_y1:zoom_y2, zoom_x1:zoom_x2]
                #Resized as per configuration 
                resized_image = cv2.resize(zoomed_region,(config['model_variant']['img_width'],config['model_variant']['img_height']))
                image_path = os.path.join(os.getcwd(),f"capture", f"captured_image{int(time.time())}.png")
